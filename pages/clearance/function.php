<?php
  if(isset($_POST['btn_add'])){
    $txt_cnum = random_string();				//changed this to function
        ...
        while($row = mysqli_fetch_array($chk)){
            ...
            if($row['lengthofstay'] < -1){
                ...
                $reqquery = mysqli_query($con, "INSERT INTO tblclearance (clearanceNo, residentid, findings, purpose, orNo, samount, dateRecorded, recordedBy, status, Expire) 
                        VALUES ('$txt_cnum', '".$_SESSION['userid']."', '', '$txt_purpose', '', '', '$date', '".$_SESSION['role']."', 'New', '$expirydate') ") or die('Error: ' . mysqli_error($con));
...
if(isset($_POST['btn_approve']))
{
    ...
    $name = $_POST['hidden_resname'];
    $purpose = $_POST['hidden_purpose'];
    $num = $_POST['hidden_num'];
    $msg = "congrats your request has been approved!";
   // $text = $txt_cnum.":".$msg;
    $txt2 = "Congratulations, " .$name. "! Your clearance for " .$purpose. " has been approved.  -From Tetuan Barangay Hall";

    
    require_once "vendor/autoload.php";

  //  $basic  = new \Vonage\Client\Credentials\Basic("83fbc579", "4v8fS3hqU77fdlZl");
    $basic  = new \Vonage\Client\Credentials\Basic("23fe3f9a", "xN6RgLpoSjjKo7r3");
    $client = new \Vonage\Client($basic);

    $response = $client->sms()->send(
        new \Vonage\SMS\Message\SMS($num, 'BrgyMngment', $txt2)		//need to change it to $num
    );

    $message = $response->current();

    if ($message->getStatus() == 0) {
        echo "The message was sent successfully\n";							//I think this doesn't show because of the header(location: server request uri)
    } else {
    ...
    if(isset($_SESSION['role'])){
        $action = 'Approved Clearance with clearance number of '.$txt_cnum;
        $iquery = mysqli_query($con,"INSERT INTO tbllogs (user, logdate, action) VALUES ('".$_SESSION['role']."', NOW(), '".$action."')");
    }
...
function random_string($length = 5)					//not sure where to put this so I placed it here
 {
...
if(isset($_POST['btn_disapprove']))
{
    ...
    if(isset($_SESSION['role'])){
        $query = mysqli_query($con, "SELECT * FROM tblclearance where id = '$txt_id' ");
        $row = mysqli_fetch_array($query);
        $action = 'Disapproved Clearance with clearance number of '.$row['clearanceNo'];
        $iquery = mysqli_query($con,"INSERT INTO tbllogs (user, logdate, action) VALUES ('".$_SESSION['role']."', NOW(), '".$action."')");
    }
